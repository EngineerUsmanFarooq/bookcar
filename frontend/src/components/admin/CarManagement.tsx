...
import { carsAPI, bookingsAPI, Car, Booking } from "@/services/api";

interface CarManagementProps {
  onStatsUpdate?: () => void;
}

const CarManagement = ({ onStatsUpdate }: CarManagementProps) => {
  ...
  const syncCarAvailability = async (carsList: Car[]) => {
    try {
      ...
      for (const car of carsList) {
        const carBookings = activeBookings.filter(booking =>
          ...
          (typeof booking.carId === 'object' && booking.carId?._id === car._id)
        );
  ...
  const adjustQuantity = async (carId, change) => {
    ...
    try {
      ...
      const allBookings = await bookingsAPI.getAll();
      const activeBookings = allBookings.filter(booking =>
        (booking.carId === carId || (typeof booking.carId === 'object' && booking.carId?._id === carId)) &&
        (booking.status === 'confirmed' || booking.status === 'pending')
      );

      const bookedCount = activeBookings.length;

      if (newQuantity < bookedCount) {
      ...
      const correctAvailable = newQuantity - bookedCount;
      ...
      setCars(prevCars =>
        prevCars.map(c => {
          if (c._id === carId) {
            return { ...c, quantity: newQuantity, available: correctAvailable };
          }
          return c;
        })
      );
